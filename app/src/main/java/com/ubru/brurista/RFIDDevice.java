...
public class RFIDDevice {
     ...
     * @param context Parameter no longer used, use {@link #RFIDDevice(SpiDevice, Gpio)} instead.
     * @param spiDevice SPI port used on the board
     * @param resetPin Pin connected to the RST pin on the RC522
     * @deprecated use {@link #RFIDDevice(SpiDevice, Gpio)} instead.
     */
    @Deprecated
    public RFIDDevice(Context context, SpiDevice spiDevice, Gpio resetPin) throws IOException {
     ...
     * @param resetPin Pin connected to the RST pin on the RC522
     */
    public RFIDDevice(SpiDevice spiDevice, Gpio resetPin) throws IOException {
        this.device = spiDevice;
        this.resetPin = resetPin;
        initializePeripherals();
    }
